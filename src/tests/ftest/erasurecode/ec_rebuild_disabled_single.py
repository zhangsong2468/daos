...
class EcDisabledRebuildSingle(ErasureCodeSingle):
    ...
    def test_ec_degrade_single_value(self):
        ...
        # Rebuild is disabled so data should not be rebuild
        self.server_managers[0].stop_ranks(
            [self.server_count - 1], self.d_log, force=True)
        time.sleep(10)

        # Read data set and verify for different EC object and different sizes
        self.read_single_type_dataset()

        # Kill the another server rank and wait for 20 seconds,Rebuild will
        # not happens because it's disabled.
        # Read and verify data with Parity 2.
        self.server_managers[0].stop_ranks(
