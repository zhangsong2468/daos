...
pool_svc_step_up_cb(struct ds_rsvc *rsvc)
{
	...
	D_ERROR("RYON: %d ?==? %d", prop->dpp_nr, DAOS_PROP_PO_NUM);
	...
	*prop_out = prop;
	...
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_SCHED) {
		...
		if (rc != 0)
			return rc;
		D_ASSERT(idx < nr);
		prop->dpp_entries[idx].dpe_type = DAOS_PROP_PO_SCRUBBER_SCHED;
		prop->dpp_entries[idx].dpe_val = val;
		idx++;
	}
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_FREQ) {
		...
		if (rc != 0)
			return rc;
		D_ASSERT(idx < nr);
		prop->dpp_entries[idx].dpe_type = DAOS_PROP_PO_SCRUBBER_FREQ;
		prop->dpp_entries[idx].dpe_val = val;
		idx++;
	}
	if (bits & DAOS_PO_QUERY_PROP_SCRUB_CRED) {
		...
		if (rc != 0)
			return rc;
		D_ASSERT(idx < nr);
		prop->dpp_entries[idx].dpe_type = DAOS_PROP_PO_SCRUBBER_CREDITS;
		prop->dpp_entries[idx].dpe_val = val;
		idx++;
	}
